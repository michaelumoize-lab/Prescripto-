  ...
  return profileData && (
    <div>
      ...
      <div className="flex flex-col gap-4 m-5">
        ...
        <div>
          <img className="w-full rounded-lg sm:max-w-64 bg-primary/80" src={profileData.image} alt="" />
        </div>

        <div className="flex-1 p-8 bg-white border rounded-lg border-stone-100">

          {/* ---Doc Info : name, degree, experirence ---- */}

          <p className="flex items-center gap-2 text-3xl font-medium text-gray-700">{profileData.name}</p>
          <div className="flex items-center gap-2 mt-1 text-gray-600">
            <p>{profileData.degree} - {profileData.speciality}</p>
            <button className="border py-0.5 px-2 rounded-full text-xs">{profileData.experience}</button>
          </div>
          ...
          <div className="flex gap-1 pt-2">
            <input disabled={!isEdit} onChange={() => setProfileData(prev => ({...prev, available: !prev.available}))} checked={profileData.available} type="checkbox" />
