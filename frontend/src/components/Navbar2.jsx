...
const Navbar = () => {
  ...
  return (
    <div className='flex items-center justify-between py-4 mb-5 text-sm border-b border-b-gray-400'>
      <img onClick={()=>navigate('/')} className='cursor-pointer w-44' src={assets.logo} alt="Prescripto Logo"/>
      ...
      <div className='flex items-center gap-4'>
        {
            token 
            ? <div className='relative flex items-center gap-2 cursor-pointer group' onClick={() => setShowMenu(!showMenu)}>
        ...
        {/* Mobile Menu */}
        <div className={` ${showMenu ? 'fixed w-full' : 'w-0 h-0'} md:hidden top-0 right-0 bottom-0 z-20 overflow-hidden bg-white transition-all`}>
            ...
            <ul className='flex flex-col items-center gap-2 px-5 mt-5 text-lg font-medium'>
                ...
                <NavLink onClick={() => setShowMenu(false)} to='/doctors'><p className='inline-block px-4 py-2 rounded'>DOCTORS</p></NavLink>
