  ...
  return (
    <div className='flex items-center justify-between py-4 mb-5 text-sm border-b border-b-gray-400'>
      ...
      <ul className='items-center hidden gap-5 font-medium md:flex'>
        <NavLink to='/'>
          <li className='py-1 font-bold text-primary'>HOME</li>
        ...
        <div className='flex gap-2 ml-2'>
          <a href="http://localhost:5174" target="_blank" rel="noopener noreferrer" className='px-3 py-1 text-[10px] transition-all border rounded-full border-zinc-400 text-zinc-600 hover:bg-zinc-600 hover:text-white'>
            ADMIN (LOCAL)
          </a>
          <a href="https://prescripto-admin-ovst.onrender.com" target="_blank" rel="noopener noreferrer" className='px-3 py-1 text-[10px] transition-all border rounded-full border-primary text-primary hover:bg-primary hover:text-white'>
            ADMIN (LIVE)
          </a>
        </div>
      </ul>
      ...
      <div className='flex items-center gap-4'>
        ...
        {/* --- Mobile Menu --- */}
        <div className={`fixed top-0 right-0 bottom-0 z-40 transition-all overflow-hidden bg-white ${showMobileMenu ? 'w-full' : 'w-0'}`}>
          
          {/* Backdrop Overlay: Closes menu when clicking the empty space */}
          {showMobileMenu && (
            <div 
          ...
          {/* Menu Content */}
          <div className='relative z-50 flex flex-col h-full bg-white'>
